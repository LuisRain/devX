...
import com.william.devx.jdbc.dialect.Dialect;
import com.william.devx.jdbc.dialect.DialectFactory;
import com.william.devx.jdbc.dialect.DialectType;
import com.william.devx.jdbc.proxy.MethodConstruction;
...
public class DevxDS implements DS {

    private static final Logger logger = LoggerFactory.getLogger(DevxDS.class);
    ...
    public int deleteAll(Class<?> entityClazz) {
        return delete(DevxSB.inst(), entityClazz);
    }

    @Override
    public int delete(SB sqlBuilder, Class<?> entityClazz) {
        EntityContainer.EntityClassInfo entityClassInfo = EntityContainer.getEntityClassByClazz(entityClazz);
        Object[] sb = ((DevxSB) sqlBuilder).build(entityClassInfo, leftDecorated, rightDecorated);
    ...
    public int enableByCode(String code, Class<?> entityClazz) {
        EntityContainer.EntityClassInfo entityClassInfo = EntityContainer.getEntityClassByClazz(entityClazz);
        return enable(DevxSB.inst().eq(entityClassInfo.codeFieldNameOpt.get(), code), entityClazz);
    }
    ...
    public <E> List<E> findAll(Class<E> entityClazz) {
        return find(DevxSB.inst(), entityClazz);
    }

    @Override
    public <E> List<E> findEnabled(Class<E> entityClazz) {
        EntityContainer.EntityClassInfo entityClassInfo = EntityContainer.getEntityClassByClazz(entityClazz);
        return find(DevxSB.inst().eq(entityClassInfo.enabledFieldNameOpt.get(), true), entityClazz);
    }

    @Override
    public <E> List<E> findDisabled(Class<E> entityClazz) {
        EntityContainer.EntityClassInfo entityClassInfo = EntityContainer.getEntityClassByClazz(entityClazz);
        return find(DevxSB.inst().eq(entityClassInfo.enabledFieldNameOpt.get(), false), entityClazz);
    }
    ...
    public long countAll(Class<?> entityClazz) {
        return count(DevxSB.inst(), entityClazz);
    }
    ...
    public <E> Page<E> paging(long pageNumber, int pageSize, Class<E> entityClazz) {
        return paging(DevxSB.inst(), pageNumber, pageSize, entityClazz);
    }

    @Override
    public <E> Page<E> pagingEnabled(long pageNumber, int pageSize, Class<E> entityClazz) {
        EntityContainer.EntityClassInfo entityClassInfo = EntityContainer.getEntityClassByClazz(entityClazz);
        return paging(DevxSB.inst()
    ...
    private Object[] packageInsert(Iterable<?> entities, boolean ignoreNullValue) {
        if (!entities.iterator().hasNext()) {
            throw Devx.E.e(StandardCode.BAD_REQUEST.toString(), new RuntimeException("Entity List is empty."));
        }
        ...
        for (Object entity : entities) {
            ...
            if (entityClassInfo.createUserFieldNameOpt.isPresent()) {
                if (Devx.context().optInfo().isPresent()) {
                    values.put(entityClassInfo.createUserFieldNameOpt.get(), Devx.context().optInfo().get().getAccountCode());
                } else {
            ...
            if (entityClassInfo.updateUserFieldNameOpt.isPresent()) {
                if (Devx.context().optInfo().isPresent()) {
                    values.put(entityClassInfo.updateUserFieldNameOpt.get(), Devx.context().optInfo().get().getAccountCode());
                } else {
            ...
            if (values.entrySet().stream()
                ...
                throw Devx.E.e(StandardCode.BAD_REQUEST.toString(), new RuntimeException("Not Null check fail."));
            }
    ...
    private Object[] packageUpdate(Object entity, boolean ignoreNullValue) {
        ...
        if (!entityClassInfo.pkFieldNameOpt.isPresent() && !entityClassInfo.codeFieldNameOpt.isPresent()) {
            throw Devx.E.e(StandardCode.NOT_FOUND.toString(), new RuntimeException("Need @PkColumn or @CodeColumn field."));
        }
        ...
        if (whereColumnName == null) {
            if (entityClassInfo.codeFieldNameOpt.isPresent()
                ...
                throw Devx.E.e(StandardCode.BAD_REQUEST.toString(), new RuntimeException("Need Private Key or Code value."));
            }
        }
        // Filter null value if ignoreNullValue=true
        if (ignoreNullValue) {
