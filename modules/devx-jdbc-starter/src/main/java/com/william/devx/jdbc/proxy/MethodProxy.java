package com.william.devx.jdbc.proxy;
        ...
        for (Annotation annotation : method.getMethodAnnotations()) {
            if (annotation instanceof Select) {
                if (method.flagOfPaging()) {
                    return ((DevxDS) Devx.ds(dsName)).selectForPaging(((Select) annotation).entityClass(), method, ((Select) annotation).value());
                }
                List list = ((DevxDS) Devx.ds(dsName)).selectForList(((Select) annotation).entityClass(), method.getParamsMap(), ((Select) annotation).value());
