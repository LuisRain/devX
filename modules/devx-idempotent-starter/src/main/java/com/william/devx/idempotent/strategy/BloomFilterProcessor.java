...
@Component
public class BloomFilterProcessor implements DevxIdempotentProcessor {

    private static final String NAME_SPACE_EXIT = "devx:idempotent:bloom:exit:";

    private static final String NAME_SPACE_CONFIRM = "devx:idempotent:bloom:confirm";
    ...
    private class BloomFilter {
        ...
        private boolean setExitRecord(String key,boolean value) {
            ...
            return Devx.cluster.cache.setBit(NAME_SPACE_EXIT + getBiteKey(hash), getOffset(hash), true);
        }

        private boolean getExitRecord(String key) {
            long hash = getHash(key);
            return Devx.cluster.cache.getBit(NAME_SPACE_EXIT + getBiteKey(hash), getOffset(hash));
        }

        private boolean setConfirmRecord(String key,boolean value) {
            long hash = getHash(key);
            return Devx.cluster.cache.setBit(NAME_SPACE_CONFIRM + getBiteKey(hash), getOffset(hash), true);
        }

        private boolean getConfirmRecord(String key) {
            long hash = getHash(key);
            return Devx.cluster.cache.getBit(NAME_SPACE_CONFIRM + getBiteKey(hash), getOffset(hash));
        }
