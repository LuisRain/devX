package com.william.devx.core.hystrix;
...
public class FailureEventNotifier extends HystrixEventNotifier {
    ...
    @PostConstruct
    public void init() {
        this.notifiedTime = Instant.now().toEpochMilli() - devxCloudConfig.getError().getNotifyIntervalSec() * 1000;
        this.notifyIncludeKeys.addAll(Arrays.asList(devxCloudConfig.getError().getNotifyIncludeKeys()));
        this.notifyExcludeKeys.addAll(Arrays.asList(devxCloudConfig.getError().getNotifyExcludeKeys()));
    }
    ...
    public void markEvent(HystrixEventType eventType, HystrixCommandKey key) {
        ...
        if (!devxCloudConfig.getError().getNotifyEventTypes().contains(eventType.name())) {
            return;
        }
        failureInfo.putIfAbsent(key.name(), new HashSet<>());
        failureInfo.get(key.name()).add(eventType.name());

        if (Instant.now().toEpochMilli() - notifiedTime < devxCloudConfig.getError().getNotifyIntervalSec() * 1000) {
            return;
        }
    ...
    private void sendEmail() {
        try {
            ...
            message.setTo(devxCloudConfig.getError().getNotifyEmails().toArray(new String[]{}));
            message.setSubject(Devx.devxConfig.getBasic().getName() + devxCloudConfig.getError().getNotifyTitle());
